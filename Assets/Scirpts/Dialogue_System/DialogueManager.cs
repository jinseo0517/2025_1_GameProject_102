...
{
    [Header("UI 요소 - Instrctor에서 연결")]
    public GameObject DialoguePanel;            //대화창 전체 패널(처음엔 비활성화)
    public Image characterImage;                //캐릭터 이미지 표시하는 이미지유아이
    public TextMeshProUGUI characternameText;   //캐릭터 이름 표시하는 텍스트
    public TextMeshProUGUI dialogueText;        //대화내용 표시하는 텍스트
    public Button nextButton;                   //"다음" 버튼 (클릭 시 다음 대화로)

    [Header("기본 설정")]
    public Sprite defaultCaracterImage;         //캐릭터 이미지가 없을 떄 사용할 기본 이미지

    [Header("타이핑 효과 설정")]
    public float typingSpeed = 0.05f;           //글자 하나당 출력 속도 (초단위)
    public bool skipTypingOnClick = true;       //클릭시 타이핑 즉시 완료 할지 여부
    ...
    public void StartDialogue(DialogueDataSO dialogue)      //새로운 대화를 시작하는 함수
    {
        ...
        if(characterImage != null)
        {
            ...
            {
                characterImage.sprite = defaultCaracterImage;       //기본 이미지 사용
            }
        }

        ShowCurrentLine();                                          //첫 번쨰 대화 내용 표시
    }
    ...
    void ShowCurrentLine()      //현재 대화줄의 내용을 타이핑 효과와 함께 화면에 표시하는 함수
    {
    ...
    public void ShowNextLine()      //다음 대화줄로 이동시키는 함수 (타이핑이 완료된 후에만 호출)
    {
    ...
    public bool IsDialogueActive()      //현재 대화가 진행중인지 확인하는 함수
    {
