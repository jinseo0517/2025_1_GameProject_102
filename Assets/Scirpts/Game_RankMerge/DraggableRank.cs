...
public class DraggableRank : MonoBehaviour
{
    ...
    public float dragSpeed = 10f;           //드래그시 이동 속도
    ...
    void Update()
    {
        ...
        else if(transform.position != originalPosition && currentCell != null)      //드래그가 끝났는데 원래 위치로 돌아가야 하는경우
        {
    ...
    void StopDragging()
    {
        ...
        if(targetCell != null)
        {
            if(targetCell.currentRank == null)      //빈 칸인 경우
            {
        ...
        {
            ReturnToOriginalPosition();     //유효한 칸이없으면 위치로 복귀
        }
    }
    ...
    public void SetRankLevel(int level)
    {
        ...
        {
            spriteRenderer.sprite = gameManager.rankSprites[level - 1];     //레벨에 맞는 스프라이트 변경
        }
    }
}
