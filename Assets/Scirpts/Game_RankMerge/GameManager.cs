...
public class GameManager : MonoBehaviour
{
    ...
    private GridCell FindEmptyCell()            //비어있는 칸 찾기
    {
        ...
        for (int x = 0; x < gridWidth; x++)                     //모든칸을 검사
        {
            for (int y = 0; y < gridHeight; y++)
            {
        ...
        if(emptyCells.Count == 0)                                 //빈칸이 없으면 null값 반환
        {
            return null;
        }
    ...
    public bool SpawnNewRank()      //새 계급장 생성
    {
        ...
        int rankLevel = Random.Range(0, 100) < 80 ? 1 : 2;        //80% 확률로 레벨1, 20% 확률로 레벨 2

        CreateRankInCell(emptyCell, rankLevel);                   //3. 계급장 생성 및설정
    ...
    public GridCell FindClosestCell(Vector3 position)       //가장 가까운 칸 찾기
    {
        ...
        if( closestDistance > cellSize * 2)     //너무 멀면 null변환
        {
            return null;
        }
    ...
    public void MergeRanks(DraggableRank draggedRank, DraggableRank targetRank)
    {
        ...
        if (newLevel > maxRankLevel)                    //최대 레벨 초과시 처리
        {
