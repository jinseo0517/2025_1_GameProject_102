...
public class GameController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Timer <= 0)                      //만약 Timer의 수치가 0이하로 내려갈 경우 (1초마다 동작되는 행동을 만들떄)
        {
            Timer = 1;                      //다시 1초로 타이버를 초기화 시켜준다.
        ...
        if (Input.GetMouseButtonDown(0))        //마우스 버튼을 눌렀을때
        {
            ...
            if (Physics.Raycast(ray, out hit))          //Ray를 쐈을떄 Hit 되는 물체가 있으면
            {
                if (hit.collider != null)               //물체가 존재 하면
                {
                    //Debug.Log($"hit : {hit.collider.name}");        //물체 이름을 출력한다.
                    hit.collider.gameObject.GetComponent<Enemy>().CharacterHit(30);  //Enemy 스크립트의 히트 함수를 호출한다.
                }
            }
        }
